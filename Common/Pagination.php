...
class Pagination
{
        ...
        if ($this->total > $this->limit) {
            ...
            if($this->amount() < 3) {
                ...
                if ($this->current_page == 1) {
                    $links = '<li class="active"><a>' . $this->current_page . '</a></li>' .
                    ...
                    $links = $this->generateHtml($this->current_page - 1, null, $this->current_page - 1) .
                        '<li class="active"><a>' . $this->current_page . '</a></li>';
                }
            ...
            if (!is_null($links)) {
                ...
                if ($this->current_page > 1)
                    ...
                    $links = /*$this->generateHtml(1, '&lt;&lt;', "На первую")
                        . */$this->generateHtml($this->current_page - 1, '&lt;', "На предыдущую") . $links;

                # Если текущая страница не последняя
                if ($this->current_page < $this->amount)
                    # Создаём ссылку "На последнюю"
                    $links .= $this->generateHtml($this->current_page + 1, '&gt;', "На следующую");
                        /*. $this->generateHtml($this->amount(), '&gt;&gt;', "На последнюю");*/
            }
    ...
    private function generateHtml($page, $text = null, $atribute = null)
    {
        ...
        return '<li><a ' . "title='$atribute'" . ' href="' . $currentURI . $page . '">' . $text . '</a></li>';
    }
